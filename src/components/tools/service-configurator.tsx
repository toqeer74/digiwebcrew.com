              ...
              {(["MVP", "Standard", "Enterprise"] as const).map((tier) => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      "p-4 rounded-2xl border transition-all flex flex-col justify-between h-24 text-left cursor-pointer",
                      isSelected 
                  ...
                  >
                    <p className={cn("text-base sm:text-lg md:text-xl font-black uppercase tracking-tight transition-colors", isSelected ? "text-slate-950 dark:text-white" : "text-slate-500 dark:text-slate-400")}>
                      {tier}
                    </p>
              ...
              {Object.keys(TECH_METADATA).map((tech) => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      "p-4 rounded-xl border text-[11px] font-bold uppercase tracking-widest transition-all flex flex-col items-center justify-center gap-2 text-center h-28 relative group cursor-pointer",
                      isSelected 
                        ...
                        : "border-slate-200 bg-white text-slate-405 hover:border-slate-300 dark:border-white/5 dark:bg-white/5 dark:text-slate-400 dark:hover:border-white/10"
                    )}
                  >
                    <div className={cn(
              ...
              {["SSL Encryption", "JWT Auth", "Two-Factor Auth", "SOC2 Compliance", "HIPAA Ready", "GDPR Guard"].map((sec) => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      "p-4 rounded-xl border transition-all flex items-center gap-4 text-[11px] font-bold uppercase tracking-[0.15em] cursor-pointer",
                      isSelected 
                        ? "border-[var(--site-primary)] bg-[var(--site-primary)]/[0.02] text-slate-950 dark:border-[var(--site-primary)] dark:bg-[var(--site-primary)]/[0.05] dark:text-white" 
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-4">
            ...
            <div className="rounded-2xl border border-slate-200/80 bg-slate-50/40 p-6 dark:border-white/10 dark:bg-white/[0.01]">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div>
                  <p className="text-[11px] font-bold uppercase tracking-[0.2em] text-slate-500 dark:text-slate-400 mb-2">Project Architecture</p>
                  <p className="text-xl font-bold text-slate-950 dark:text-white mb-4">{config.tier} Infrastructure</p>
                  <p className="text-[11px] font-bold uppercase tracking-[0.2em] text-slate-500 dark:text-slate-400 mb-2">Technical Core</p>
                    ...
                    {config.stack.map((s) => {
                       ...
                       return (
                        <div key={s} className="flex items-center gap-2 rounded-lg bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 px-2 py-1.5 shadow-none">
                          ...
                          <span className="text-[11px] font-bold uppercase tracking-wider text-slate-700 dark:text-slate-300">{s}</span>
                        </div>
                       );
                    })}
                  </div>
                ...
                <div>
                  <p className="text-[11px] font-bold uppercase tracking-[0.2em] text-slate-500 dark:text-slate-400 mb-2">Security Standard</p>
                  <div className="flex flex-wrap gap-2 mb-4">
                    {config.security.map((s) => (
                      <span key={s} className="px-2 py-1 bg-emerald-500/10 text-emerald-500 rounded-lg text-[11px] font-bold uppercase tracking-wider">{s}</span>
                    ))}
                  </div>
                </div>
              </div>
    ...
    <div className="relative h-full flex flex-col overflow-hidden rounded-[20px] border border-slate-200/80 bg-gradient-to-br from-white to-slate-50/50 backdrop-blur-xl p-5 md:p-6 shadow-sm transition-all duration-500 hover:border-slate-300 hover:shadow-md dark:border-white/10 dark:from-slate-900/40 dark:to-slate-900/80">
      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        <div>
          ...
          <p className="text-[11px] font-bold uppercase tracking-[0.16em] text-slate-600 dark:text-slate-400 mt-0.5">Configure your industrial scope</p>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col justify-center min-h-[210px]"><AnimatePresence mode="wait">{renderStep()}</AnimatePresence></div>

      <div className="mt-5 flex items-center justify-between border-t border-slate-200 dark:border-white/10 pt-5">
        <button
          type="button"
          onClick={prevStep}
          disabled={step === 1}
        ...
        {step < 4 && (
          <button type="button" onClick={nextStep} className="group relative flex items-center gap-3 rounded-full bg-slate-950 px-6 py-3 text-xs font-bold uppercase tracking-wider text-white transition-all hover:bg-slate-900 dark:bg-white dark:text-slate-950 dark:hover:bg-slate-50 cursor-pointer shadow-md hover:shadow-lg active:scale-[0.98]">
            Next Level
            <ArrowRight size={16} className="transition-transform group-hover:translate-x-1" />
          </button>
        )}
