      ...
      <main className="flex-1">
        ...
        <section className="relative pt-32 pb-24 md:pt-48 md:pb-32 overflow-hidden bg-transparent text-slate-900 dark:text-white">
          ...
          <div className="absolute inset-0 -z-10 overflow-hidden">
            <div className="absolute top-0 right-0 w-full h-full bg-gradient-to-br from-[var(--site-primary)]/20 via-transparent to-[rgba(var(--site-primary-rgb),0.1)] opacity-30" />
            <div className="absolute -bottom-24 -left-24 w-96 h-96 bg-[var(--site-primary)]/20 blur-[150px] rounded-full" />
          ...
          <Container>
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
              <AnimatedSection direction="right">
                <div className="inline-flex items-center gap-4 px-10 py-4 rounded-full bg-[var(--site-primary)]/10 text-[var(--site-primary)] border border-[var(--site-primary)]/30 mb-8 text-[11px] font-bold font-body uppercase tracking-[0.4em] backdrop-blur-md">
                  <Icons.Sparkle size={18} strokeWidth={2.5} className="text-[var(--site-primary)]" />
                ...
                <h1 className="text-6xl md:text-8xl font-display font-black tracking-tighter mb-8 leading-[0.9]">
                  ...
                  <span className="bg-gradient-to-r from-[var(--site-primary)] to-[var(--site-primary-soft)] bg-clip-text text-transparent block">{tech.title.split(' ').slice(-1)}</span>
                </h1>
                ...
                <Button
                  ...
                  variant="secondary"
                  className="group !rounded-xl !bg-white !text-midnight hover:!bg-gray-50 hover:scale-[1.02] active:scale-95 !border-white font-display font-extrabold text-[13px] px-10 uppercase tracking-[0.2em] dark:shadow-lg dark:shadow-[var(--site-primary)]/10 w-max"
                >
              ...
              <AnimatedSection direction="left" className="relative hidden lg:block">
                <div className="aspect-square glass border-[var(--site-primary)]/10 rounded-[4rem] flex items-center justify-center relative overflow-hidden bg-[var(--site-primary)]/5">
                  <Icon size={240} className="text-[var(--site-primary)]/10" strokeWidth={0.5} />
                  {/* Animated Pulse */}
                  <div className="absolute inset-0 flex items-center justify-center">
                    <motion.div
                      animate={{ scale: [1, 1.5, 1], opacity: [0, 0.2, 0] }}
                      transition={{ duration: 4, repeat: Infinity }}
                      className="w-64 h-64 border-2 border-[var(--site-primary)]/50 rounded-full"
                    />
                  </div>
                </div>
              </AnimatedSection>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {tech.capabilities.map((cap: string, i: number) => (
                <AnimatedSection
                  key={i}
                  delay={i * 0.1}
                  className="site-card site-card-interactive p-10 text-center group"
                >
                  <div className="w-16 h-16 rounded-2xl bg-[var(--site-primary)]/5 flex items-center justify-center text-[var(--site-primary)] mx-auto mb-6 group-hover:scale-110 transition-transform">
                    <Target size={32} />
                  </div>
              ...
              {tech.relatedServices.map((serviceSlug: string) => {
                ...
                return (
                  <Link
                    ...
                    className="site-card site-card-interactive group p-8 flex items-center justify-between"
                  >
                    <div>
                      <p className="text-[10px] font-black uppercase tracking-widest text-[var(--site-primary)] mb-2">Service Category</p>
                      <h4 className="text-2xl font-bold tracking-tight">{title}</h4>
                    </div>
                    <div className="w-12 h-12 rounded-xl bg-slate-100 dark:bg-midnight border border-slate-200 dark:border-white/10 flex items-center justify-center text-slate-900 dark:text-white group-hover:bg-slate-200 dark:group-hover:bg-white/10 group-hover:border-[var(--site-primary)]/50 transition-all dark:shadow-sm">
        ...
        {/* Value Props */}
        <Section className="py-24 md:py-32 bg-[var(--site-primary)]/5">
          <Container>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
              {[
                ...
                <AnimatedSection key={i} delay={i * 0.1} className="space-y-4">
                  <item.icon size={40} className="text-[var(--site-primary)]" strokeWidth={1} />
