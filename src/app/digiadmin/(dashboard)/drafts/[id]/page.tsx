...
export default async function DraftDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!session) redirect("/digiadmin/login");
  ...
  if (!draft) redirect("/digiadmin/drafts");
  ...
  return (
    <div className="admin-page-stack w-full pb-8">
      <PageHeader
        ...
        breadcrumb={[
          { label: "Dashboard", href: "/digiadmin/dashboard" },
          { label: "Drafts", href: "/digiadmin/drafts" },
          { label: "Detail" },
        ]}
        actions={
          <div className="flex items-center gap-2">
            ...
            {d.workflowRunId && (
              <Link href={`/digiadmin/workflow-runs/${d.workflowRunId}`} className="adm-btn adm-btn-secondary adm-btn-sm">
                View Run
              </Link>
            )}
            <Link href="/digiadmin/drafts" className="adm-btn adm-btn-secondary adm-btn-sm inline-flex items-center gap-2">
        ...
        <ACard className="lg:col-span-4">
          ...
          <ACardBody className="space-y-4">
            ...
            <div>
              ...
              {d.workflowRunId ? (
                <Link
                  href={`/digiadmin/workflow-runs/${d.workflowRunId}`}
