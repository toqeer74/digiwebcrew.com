        ...
        <div className="max-w-6xl mx-auto space-y-24">
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
            <AnimatedSection className="md:col-span-5 site-card p-10 relative overflow-hidden bg-slate-50 dark:bg-white/5 border-slate-200">
              ...
              <h2 className="text-2xl font-display font-black text-slate-950 dark:text-[#F8F8FF] mb-6 tracking-tight uppercase tracking-widest text-xs opacity-50">Support Tiers</h2>
            ...
            <AnimatedSection className="md:col-span-7 site-card p-10 relative overflow-hidden bg-slate-950 text-white flex flex-col">
               <h3 className="text-3xl font-display font-black mb-8">Performance & Security Outcomes</h3>
               <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
                 {outcomes.map(o => (
                   <div key={o}>
                     <p className="text-emerald-400 font-black text-lg mb-2">{o.split(' ')[0]}</p>
                     <p className="text-slate-400 text-xs font-bold uppercase tracking-wider">{o.split(' ').slice(1).join(' ')}</p>
                   </div>
                 ))}
               </div>
               <div className="mt-auto pt-10 border-t border-white/10 flex flex-wrap gap-4">
                  {techStack.map(t => (
                    <span key={t} className="text-[10px] font-black uppercase tracking-widest text-slate-500">{t}</span>
                  ))}
               </div>
            </AnimatedSection>
          </div>
