        ...
        <div className="max-w-6xl mx-auto space-y-24">
          ...
          <AnimatedSection className="text-center flex flex-col items-center">
            ...
            <AnimatedSection className="md:col-span-12 site-card p-10 lg:p-12 relative overflow-hidden">
              ...
              <h2 className="text-3xl font-display font-black text-slate-950 dark:text-[#F8F8FF] mb-6 tracking-tight">Enterprise Commerce Ops</h2>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                {specs.map((s, i) => (
                  <div key={i} className="site-card-interactive p-6 flex flex-col gap-3 group border-2 border-transparent hover:border-[var(--site-primary)]/20">
                    ...
                    <span className="text-sm font-black text-slate-950 dark:text-[#F8F8FF] uppercase tracking-tight">{s}</span>
                  </div>
                ))}
              </div>
            </AnimatedSection>
            ...
            <AnimatedSection className="md:col-span-12 site-card p-10 lg:p-12 relative overflow-hidden">
               ...
               <h3 className="text-2xl font-display font-black text-slate-950 dark:text-[#F8F8FF] mb-8">Modern High-Conversion Stack</h3>
               ...
               <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6">
                 {features.map(f => (
                   <div key={f} className="p-6 rounded-2xl bg-slate-50 dark:bg-white/5 border border-slate-100 dark:border-white/10 flex items-center gap-3">
                     ...
                     <p className="font-bold text-sm text-slate-950 dark:text-[#F8F8FF] uppercase tracking-tight">{f}</p>
                   </div>
                 ))}
               </div>
            </AnimatedSection>
